...
async function saveSettings() {
  ...
  try { await api.runtime.sendMessage({ type: "reschedule" }); } catch (_e) {}
  const st = await api.storage.local.get(["current", "lastCheck", "lastError"]);
  renderStatus(st);
...
$("allDeals").addEventListener("click", (e) => {
  e.preventDefault();
  api.tabs.create({ url: api.runtime.getURL("deals.html") });
});
$("checkNow").addEventListener("click", async () => {
  $("status").textContent = "Prüfe …";
  try { await api.runtime.sendMessage({ type: "checkNow" }); } catch (_e) {}
  await load();
});
